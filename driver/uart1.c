...
#include "string.h"
 ...
 volatile uint8_t rx_buffer[RX_LENGTH];
 volatile uint16_t rx_len;
  uint8_t tx_buffer[TX_LENGTH];


static MASTER_RECV_DEALWITH recvFun = 0;	//定义一个函数指针

//====================================
//接收数据处理，与bsp层交互
//====================================
static uint8_t recv_data_dealwith(volatile uint8_t *buf,volatile uint16_t len,MASTER_RECV_DEALWITH callBack)
{
	if(callBack == 0)
		return 2;
	if(callBack(buf,len))
...
void usart1_config(void)
{
	...
	GPIO_Init(GPIOA, &GPIO_InitStructure);
	//串口配置
	USART_InitStructure.USART_BaudRate            = MASTER_COMBAUD  ;				//波特率
...
{
	memcpy(tx_buffer, dat, size);//拷贝数据到发送区
    DMA_Cmd(DMA1_Channel4, DISABLE);//??DMA??
...
void USART1_IRQHandler(void)
{
	...
	if(USART_GetITStatus(USART1, USART_IT_IDLE) != RESET)//空闲中断
	{
		...
		if(rx_len != 0)
		{
			recv_data_dealwith(rx_buffer,rx_len,recvFun);
		}
		else
		{
			sr = sr;
